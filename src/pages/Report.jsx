...
function Report() {
  ...
  // Effect for initial static data (stats and users list)
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        setError('Statistika və istifadəçi məlumatlarını yükləmək mümkün olmadı.');
        console.error(err);
      }
    };
    fetchInitialData();
  ...
  // Effect to fetch logs whenever filters change
  useEffect(() => {
    const fetchLogs = async () => {
      ...
      try {
        ...
        const logsResponse = await reportsAPI.getActivityLogs(params);
        setLogs(logsResponse.data.results || []);

      } catch (err) {
        setError('Fəaliyyət tarixçəsini yükləmək mümkün olmadı. Zəhmət olmasa, şəbəkə bağlantınızı yoxlayın.');
        console.error(err);
      } finally {
        setLoading(false);
      }
