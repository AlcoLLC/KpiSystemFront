...
useEffect(() => {
    ...
    return (
        <Modal
            ...
            open={open} onCancel={onCancel} onOk={() => form.submit()} width={1000} destroyOnClose
            ...
            <Form form={form} layout="vertical" onFinish={submit} disabled={mode === 'view'}>
                ...
                {selectedVolumes.map((vol, idx) => (
                    <Row gutter={12} key={vol.id} align="middle" className="mb-2">
                        <Col span={4}><Text strong>{vol.volume} L</Text><Form.Item name={['items', idx, 'volume']} initialValue={vol.id} hidden><Input/></Form.Item></Col>
                        <Col span={4}><Form.Item name={['items', idx, 'production_hours']} noStyle><InputNumber min={0} max={8} className="w-full" placeholder="Saat"/></Form.Item></Col>
                ...
                <Form.Item name="note" label="Qeyd"><Input.TextArea rows={2}/></Form.Item>
            </Form>
        </Modal>
    );
};
